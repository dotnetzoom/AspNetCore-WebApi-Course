...
namespace WebFramework.Swagger
{
    public class ApplySummariesOperationFilter : IOperationFilter
    {
        public void Apply(OpenApiOperation operation, OperationFilterContext context)
        {
            ...
            string singularizeName = pluralizer.Singularize(controllerActionDescriptor.ControllerName);
            string pluralizeName = pluralizer.Pluralize(singularizeName);

            int parameterCount = operation.Parameters.Where(p => p.Name != "version" && p.Name != "api-version").Count();
            ...
            bool IsGetAllAction()
            {
                foreach (string name in new[] { "Get", "Read", "Select" })
                {
